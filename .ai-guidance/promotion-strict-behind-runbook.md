...
2. **Get fresh authorization, then disable strict.** This is its own sensitive action — weakening branch protection — and is gated separately from the promotion/merge approval itself. A prior "approve push" or "promote to main" in the conversation does **not** satisfy this gate by design: ask the human explicitly (e.g. "this requires disabling branch protection on `<branch>` — say 'approve strict-disable' to proceed") and wait for a fresh reply containing that phrase before running the command below. `tools/claude-hooks/pre-tool-use-red-autonomy.sh` blocks the command with exit 2 if this approval isn't present in the current session.
   ```bash
