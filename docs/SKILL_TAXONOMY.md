...
| **superpowers-plus additions** | 96 net-new skills covering engineering, wiki, security, research, and more |

110 skills total. Count verified against `find skills -name skill.md | wc -l`; per-domain breakdown in [Domain Reference](#domain-reference) below.
...
*110 skills across 9 domains (9 overrides, 5 base, 96 net-new). Counts verified against the filesystem, not carried forward from an earlier snapshot. Re-run `find skills -name skill.md | wc -l` and recount this table's domain rows before trusting either number again.*
