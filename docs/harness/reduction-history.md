...
|---|---|---|---|---|---|---|---|
| progressive-harsh-review | 18018 | 8493 | 1602 | 10095 | 7923 (43%) | 52% | Persona weights, vetoes, verdicts, convergence, anti-recursion, the sentinel rule, AND the verbatim `## Failure Modes` table stay resident; project-floor detail and the report format load from `reference.md` through trusted managed tooling. The Failure Modes table was moved BACK into the kernel on review: loading it on demand put the anti-rubber-stamping rules (the <=7 score-inflation cap, the REGRESSION flag, Operational-Risk veto eligibility) behind the exact condition they exist to prevent. Deleted: the three long-form `### Persona N` lens definitions and `## Artifact-Aware Persona Mapping`, condensed into one persona dimension table per the approved Phase 2A plan. 2026-09-20: llm-skill-review found the kernel-split loader's installed-path precedence was reversed (`.agents`->`.codex`->`.claude` instead of the canonical `.claude`->`.codex`->`.agents`) and bound the wrong loader to a source-checkout fallback -- both fixed by replacing the block with the exact canonical template debate/context-ferry already use (also removes an unguarded `cmp` dependency that hard-failed on the sanctioned `--skip-augment` staleness state). Also fixed a stale "detailed failure recovery" phrase in the Reference loading prose that resolved to no real heading. 2026-09-21: persona count made explicit (all three only for irreversible artifacts, else SeniorArchCritic alone; unsure -> three) and re-review continues the same reviewer on the delta. Kept under the 8,500 budget by removing two Failure Modes rows that restated Step 8 and the REJECT verdict row, and by replacing the now-false "sentinel binds to HEAD" line with the content-scoped rule from `tools/lib/sentinel-scope.sh`. |
