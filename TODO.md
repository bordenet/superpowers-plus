...
- [x] [20260919-03] **`tools/router-precision.py`'s `analyze()` should move off its positional-tuple return to a dataclass** — DONE 2026-09-20, commit `539b75b8`. `analyze()` now builds and returns the full report dict (incl. `"skipped"`) directly instead of a positional tuple; the metrics-writer heredoc in `user-prompt-submit-skill-router.sh` also moved from 8 positional argv values to named env vars (same finding class). Verified: router-precision-test.bats 18/18, claude-guardrails-test.bats 151/151, full suite green.
- [ ] [20260920-01] **Resolve the Cursor cross-agent compatibility claim** — `llm-skill-review` (2026-09-20), Cross-Agent Compatibility Critic. The kernel-split loader probes three candidate dirs (`$HOME/.claude/skills`, `$HOME/.codex/skills`, `$HOME/.agents/skills`), but those are Claude Code, superpowers-plus canonical, and Augment IDE — not three independent vendors. Meanwhile `skills/engineering/llm-skill-review/skill.md:82` advertises “review for Claude Code/Augment Code/Cursor compatibility”, and the only Cursor reference in the whole repo is transcript discovery in `pre-tool-use-red-autonomy.sh` (`~/.cursor/projects/`). There is no Cursor skill-install path, so a kernel-split skill installed under Cursor falls through to the source-checkout fallback or fails to load its `reference.md`. **Deferred, not fixed:** the correct fix needs external research into Cursor’s actual skill/rules install convention — inventing a plausible-looking `$HOME/.cursor/skills` would be worse than having none. Two honest options: (a) research the real path and add it to the canonical loader template + `deploy.sh`, or (b) drop the Cursor claim and state the supported set as Claude Code + Augment only.
