...
| outline-wiki-guardrails skill | Created (454 words) | **612 words** (in superpowers-[company]) | ✅ |
| linear-guardrails skill | Created (214 words) | **397 words** (in superpowers-[company]) | ✅ |
| fathom-api skill | Created (123 words) | **257 words** (in superpowers-[company]) | ✅ |
...
- Phase 6 on-demand skills were created in **superpowers-[company]** (private [Company] overlay), not superpowers-plus. This is correct — they contain [Company]-specific configuration (Linear team IDs, Outline wiki URLs, Fathom API keys).
- The PRD's original always-rules baseline of 12 files referred to superpowers-[company] rules, not superpowers-plus (which only had 4). After consolidation, superpowers-[company] also has a single `core.always.md`.
