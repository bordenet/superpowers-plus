...
Not sure which gate applies to a given file, or which review skill to dispatch? Don't work it out from memory — run `tools/review.sh route <path> [<path> ...]` (or `tools/which-gate.sh <path> [<path> ...]` for just the gate lookup without the skill/runner translation). It answers by extracting and running the actual detection logic from the gate scripts themselves, so it can't drift out of sync with them the way hand-written prose (this list included) has a way of doing.
