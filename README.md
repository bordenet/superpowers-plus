...
3. Restart your client. Verify: run `find_skills` in the MCP client — expected output lists ~92 available skill names.
...
# Expected: ~92 skill names listed
```
...
| Stale skill count | `bash install.sh --upgrade`; verify with `node ... find-skills \| wc -l` — expect ~92 |
