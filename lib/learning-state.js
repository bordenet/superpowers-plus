...
function recordOutcome(skill, outcome, evidence = '', triggerPhrase = '') {
  ...
  const explicitOutcomes = metrics.successes + metrics.failures;
  metrics.success_rate = explicitOutcomes > 0 
    ? metrics.successes / explicitOutcomes 
    : 0;
...
/**
 * Record a skill fire (invocation) without recording an outcome.
 * Use this when a skill is loaded — the outcome (success/failure) is
 * determined later by the agent after skill-guided work completes.
 * @param {string} skill - The skill name
 * @param {string} triggerPhrase - Optional trigger phrase
 * @returns {string} The fire event ID
 */
function recordFire(skill, triggerPhrase = '') {
  const state = readState();
  const id = `fire-${Date.now()}`;

  // Add to outcomes array as 'fired' (not success/failure)
  state.outcomes.push({
    id,
    skill,
    timestamp: new Date().toISOString(),
    trigger_phrase: triggerPhrase,
    outcome: 'fired',
    evidence: 'auto-logged: skill loaded via use-skill'
  });
  ...
  // Update trigger metrics — increment fires only, not success/failure
  if (!state.trigger_metrics[skill]) {
    state.trigger_metrics[skill] = {
      total_fires: 0,
      successes: 0,
  ...
  const metrics = state.trigger_metrics[skill];
  metrics.total_fires++;
  // Don't update success_rate — that's only from explicit outcomes

  // Track trigger phrases
  if (triggerPhrase && !metrics.common_triggers.includes(triggerPhrase)) {
    metrics.common_triggers.push(triggerPhrase);
    if (metrics.common_triggers.length > 10) {
      metrics.common_triggers.shift();
