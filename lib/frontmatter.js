...
function parseFrontmatter(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (inComposition) {
            // YAML list item under a composition sub-key:  "    - value"
            const listItem = line.match(/^    - (.+)$/);
            if (listItem && compListKey) {
                if (!Array.isArray(result.composition[compListKey])) {
                    result.composition[compListKey] = [];
                }
                result.composition[compListKey].push(listItem[1].trim().replace(/^["']|["']$/g, ''));
                continue;
            }
            // Sub-key:  "  key: value" or "  key:"
            if (line.match(/^  [a-zA-Z_]+:/)) {
                compListKey = null; // reset list accumulator
                const compMatch = line.match(/^  ([a-zA-Z_]+):\s*(.+)?$/);
                if (compMatch) {
                    ...
                    if (!value) {
                        ...
                        result.composition[key] = true;
                    } else if (value === 'false') {
                        result.composition[key] = false;
                    } else if (!isNaN(value) && value !== '') {
                        result.composition[key] = parseInt(value, 10);
                    } else {
                        result.composition[key] = value;
                    }
                }
            ...
            // Not indented enough — end of composition block
            if (!line.match(/^  /)) {
                inComposition = false;
                compListKey = null;
            }
