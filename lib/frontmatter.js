 ...
 * - requires_mcp arrays
 * - compress and disable-model-invocation flags
...
/** Parse YAML boolean spellings accepted by Claude frontmatter. */
...
function parseFrontmatter(content) {
    const result = {
        ...
        compress: true, disable_model_invocation: false
    };
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (match) {
            ...
            if (key === 'disable-model-invocation') {
                result.disable_model_invocation = parseYamlBoolean(value);
            }
...
function stripFrontmatter(content) {
    const lines = content.replace(/\r\n/g, '\n').replace(/\r/g, '\n').split('\n');
    // Frontmatter can only OPEN on the first line. Without this guard the first
    // bare `---` horizontal rule in a frontmatter-less document was treated as
    // an opening delimiter and everything up to the next `---` was silently
    // discarded -- measured at 67% (438 lines) of one reference.md, including
    // every section heading in it. Anything downstream that counts or hashes
    // the stripped body then cannot see that content at all.
    const hasFrontmatter = lines.length > 0 && lines[0].trim() === '---';
    if (!hasFrontmatter) return lines.join('\n').trim();
...
function validateFrontmatter(fm, source) {
    ...
    // Guard: null/undefined/non-object input
    if (!fm || typeof fm !== 'object' || Array.isArray(fm)) {
        warnings.push(`Invalid frontmatter input${src} (expected object, got ${fm === null ? 'null' : typeof fm})`);
        return warnings;
    }
    ...
    if (fm.composition !== null && fm.composition !== undefined) {
        if (typeof fm.composition !== 'object' || Array.isArray(fm.composition)) {
            ...
            for (const key of Object.keys(fm.composition)) {
                if (!validCompKeys.has(key) && !key.startsWith('x_')) {
                    // Unknown keys are forward-compat noise — only surface in DEBUG mode
                    if (process.env.DEBUG) {
            ...
            if (fm.composition.priority !== undefined && !Number.isFinite(fm.composition.priority)) {
                warnings.push(`composition.priority must be a finite number${src}`);
            }
    ...
    if (fm.coordination !== null && fm.coordination !== undefined) {
        if (typeof fm.coordination !== 'object' || Array.isArray(fm.coordination)) {
            ...
            for (const key of Object.keys(fm.coordination)) {
                if (!validCoordKeys.has(key) && !key.startsWith('x_')) {
                    // Unknown keys are forward-compat noise — only surface in DEBUG mode
                    if (process.env.DEBUG) {
            ...
            if (fm.coordination.order !== undefined && !Number.isFinite(fm.coordination.order)) {
                warnings.push(`coordination.order must be a finite number${src}`);
            }
    ...
    if (typeof fm.disable_model_invocation !== 'boolean') {
        warnings.push(`"disable-model-invocation" must be a boolean${src}`);
    }
