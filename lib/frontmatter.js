 ...
 * Those files still have their own inline parsers (with accumulator logic).
 * This file exists to:
 ...
 *
 * KNOWN LIMITATION — bracket-multiline arrays are NOT supported:
 *   triggers: ["a",
 *     "b"]           ← opening and closing bracket on different lines
 * Inline regex matching (/^field:\s*(\[.+\])\s*$/) requires both bracket
 * tokens on the same line. Bracket-multiline is handled by the accumulator-
 * based parsers in superpowers-augment.js and mcp/superpowers-mcp.js.
 * If bracket-multiline support is needed here, add accumulator logic and
 * mirror the malformed-bracket guard (/^\w+:(?:[^/]|$)/) from those files.
 * (That regex covers "key: v", "key:v", and "key:" while excluding http://...)
 *
...
function parseInlineArray(str) {
    ...
    while (i < str.length) {
        ...
        if (str[i] === '"' || str[i] === "'") {
            const quote = str[i]; i++;
            let item = '';
            while (i < str.length && !(str[i] === quote && !(quote === "'" && i + 1 < str.length && str[i + 1] === "'"))) {
                if (quote === '"' && str[i] === '\\' && i + 1 < str.length) {
                    if (str[i + 1] === quote) { item += quote; i += 2; }
                    else if (str[i + 1] === '\\') { item += '\\'; i += 2; }
                    else { item += str[i]; i++; }
                } else if (quote === "'" && str[i] === "'" && i + 1 < str.length && str[i + 1] === "'") {
                    item += "'"; i += 2; // YAML '' escape
                } else { item += str[i]; i++; }
            }
            i++;
            items.push(item);
...
function parseYamlList(lines, startIndex) {
    ...
    while (i < lines.length) {
        ...
        if (itemMatch) {
            values.push(extractStringValue(itemMatch[1]));
            i++;
        } else {
            break;
        }
...
function extractStringValue(raw) {
    ...
    if (raw.startsWith("'") && raw.endsWith("'")) {
        return raw.slice(1, -1).replace(/''/g, "'"); // YAML '' escape
    }
