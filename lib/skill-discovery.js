    ...
    for (const entry of entries) {
        ...
        if (!isDir && entry.isSymbolicLink()) {
            try { isDir = fs.statSync(skillDir).isDirectory(); }
                ...
                if (err.code === 'ENOENT' || err.code === 'ELOOP') continue; // silent skip (pre-existing behaviour)
                if (err.code === 'EACCES') {
                    process.stderr.write(`[skill-discovery] ⚠ skipping ${skillDir}: ${err.code}\n`);
                    continue;
                }
                throw err;
            }
        ...
        if (skillFile) {
            ...
            let fileSize = 0;
            try { fileSize = fs.statSync(skillFile).size; }
            catch (e) {
                if (e.code === 'ENOENT' || e.code === 'ELOOP' || e.code === 'EACCES') {
                    process.stderr.write(`[skill-discovery] ⚠ skipping ${skillFile}: ${e.code}\n`);
                    continue;
                }
                throw e;
            }
