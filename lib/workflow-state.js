...
const FEATURE_DEV_PHASES = [
    'brainstorming', 'think-twice', 'debate',
 ...
 *
 * Returns true ONLY when this process atomically created (or reaped + stole
 * an expired) lock file. Returns false in EVERY other case: another process
 * holds a fresh lock, any non-EEXIST openSync error (EACCES, EROFS, ENOSPC,
 * EISDIR, ...), or the existing lock file is unreadable. Fail-closed is the
 * correct contract for an advisory lock that guards a renameSync write:
 * returning true on indeterminate state would let two writers race and one
 * silently overwrite the other's renameSync result.
 *
 * Lock auto-expires after LOCK_TTL_MS to prevent deadlocks from crashed
 * processes. Set WORKFLOW_DEBUG=1 to surface the underlying err.code on
...
function acquireLock() {
    ...
    try {
        ...
        // Fail closed on any non-EEXIST error: unknown errors (EACCES, EROFS,
        // ENOSPC, EISDIR, etc.) leave the lock state indeterminate. Returning
        // true here would claim ownership without ever writing the lock file,
        // letting two writers clobber each other's renameSync (atomic rename
        // prevents torn writes but not lost updates).
        if (err.code !== 'EEXIST') {
            if (process.env.WORKFLOW_DEBUG) {
                console.error('workflow-state: acquireLock failed (fail-closed):', err.code || err.message);
            }
            return false;
        }
        ...
        try {
            ...
            // Lock file exists but is unreadable (corrupted/partial write).
            // Fail closed: don't risk overwriting another process's state.
            return false;
        }
    }
}
...
module.exports = {
    // --- Actively wired (called by superpowers-augment.js / hooks) ---
    recordSkillInvocation,   // superpowers-augment.js use-skill command
    getStatus,               // superpowers-augment.js bootstrap (active workflow display)
    getStateAgeHours,        // superpowers-augment.js doctor (stale state check)
    canCommit,               // tools/pre-commit hook (WORKFLOW_ENFORCEMENT=enforce)

    // --- Enforcement API (opt-in via WORKFLOW_ENFORCEMENT=enforce) ---
    // These are designed, tested, and awaiting full agent loop wiring.
    // They become active when enforcement mode is enabled.
    initWorkflow, recordGate, checkGate, checkCompletionGate,
    canClaimDone, recordFilesChanged, clearWorkflow,

    // --- Internal / testing ---
    readState, writeState, archiveState, acquireLock, releaseLock,
