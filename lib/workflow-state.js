...
const COMPLETION_GATES = ['code_review_dispatched', 'output_verified'];
...
function recordSkillInvocation(skillName) {
    ...
    // Gate-relevant skills
    const gateSkills = {
        'progressive-code-review-gate': 'code_review_dispatched',
        'code-review-battery': 'code_review_dispatched',
        'output-verification': 'output_verified',
        'verification-before-completion': 'completion_verified'
    };
    if (gateSkills[skillName]) {
        state.gates[gateSkills[skillName]] = {
            passed: true, timestamp: new Date().toISOString(), tool: skillName
        };
    }
    writeState(state);
    return state;
...
function canClaimDone() {
    ...
    if (state.files_changed && state.files_changed.length > 0) {
        for (const gate of COMPLETION_GATES) {
            if (!state.gates[gate] || !state.gates[gate].passed) missing.push(gate);
        }
    }
    return {
