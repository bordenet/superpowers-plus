...
function buildTfIdfIndex(skills) {
  ...
  for (const doc of documents) {
    ...
    // Object.create(null): doc.tfidf is later read by bracket-lookup with
    // arbitrary query terms in tfidfSimilarity's dot-product loop -- a plain
    // {} would resolve doc.tfidf['constructor'] to Object.prototype.constructor
    // (truthy) for any doc that doesn't actually contain that term, poisoning
    // the dot product with NaN for every query containing the word "constructor".
    doc.tfidf = Object.create(null);
...
function tfidfSimilarity(queryTerms, doc, df, N) {
  ...
  // This used a plain {} while buildTfIdfIndex's tf/df
  // accumulators (above) were already Object.create(null) specifically to
  // avoid prototype pollution with terms like "constructor" -- the fix was
  // never applied here. A query containing the word "constructor" resolved
  // queryTf['constructor'] to Object.prototype.constructor (the Object
  // function itself, truthy), so `(queryTf[term] || 0) + 1` string-
  // concatenated a function to a number instead of counting from 0,
  // Math.log() on that string produced NaN, and NaN silently poisoned the
  // magnitude and every downstream similarity score for the whole query.
  const queryTf = Object.create(null);
  for (const term of queryTerms) {
    queryTf[term] = (queryTf[term] || 0) + 1;
  }

  const queryTfidf = Object.create(null);
...
function expandQueryTerms(terms) {
  ...
  for (const term of terms) {
    // Must use Object.hasOwn(): CONCEPT_EXPANSIONS is a plain {} literal, so
    // CONCEPT_EXPANSIONS['constructor'] resolves via the prototype chain to
    // Object.prototype.constructor (the Object function, truthy but not
    // iterable) instead of undefined -- crashing the `for...of` below with
    // "TypeError: expansions is not iterable" on any query containing the
    // ordinary word "constructor" (or toString/hasOwnProperty/valueOf/etc).
    const expansions = Object.hasOwn(CONCEPT_EXPANSIONS, term) ? CONCEPT_EXPANSIONS[term] : undefined;
    if (expansions) {
...
// Word-boundary-aware substring matching for the anti_trigger veto ONLY.
// Raw `.includes()` matches a query term as a substring of an unrelated
// longer word (e.g. an anti_trigger phrase like "/sp-graded-review" matching
// inside an unrelated longer command). `\b` requires a transition between a
// word character and a non-word character, so it never matches at a
// boundary where BOTH sides are non-word characters -- meaning a naive `\b`
// + phrase + `\b` silently fails to match any phrase that starts or ends
// with punctuation. Apply `\b` only on the side(s) where the phrase itself
// starts/ends with a word character. A hard exclusion (this is a veto, not a
// score boost) should favor precision over recall, so this stays a strict
// literal match rather than the stem-aware matching below.
function escapeRegex(s) {
  return s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}
function wordBoundaryIncludes(haystackLower, phraseLower) {
  if (!phraseLower) return false;
  const left = /^[a-z0-9]/i.test(phraseLower[0]) ? '\\b' : '';
  const right = /[a-z0-9]$/i.test(phraseLower[phraseLower.length - 1]) ? '\\b' : '';
  return new RegExp(left + escapeRegex(phraseLower) + right, 'i').test(haystackLower);
}

// Stem-aware phrase matching for score BOOSTS (intent patterns, name/trigger/
// description matching): ports the bash/Python hook's stems_match()
// (tools/claude-hooks/user-prompt-submit-skill-router.sh) verbatim rather
// than applying this file's own generic stem() to both sides and comparing
// equality -- that weaker approach was tried first and doesn't actually
// recover "debug"/"debugging" (stem("debug")="debug" but
// stem("debugging")="debugg", so a plain equality check still misses the
// consonant-doubling case; stems_match() below compares the two raw words
// directly via startsWith+suffix-membership plus a doubled-consonant rule,
// which is what actually handles it). A raw `\b`-anchored literal match
// (tried first, then reverted) broke ordinary inflected English -- "crashed",
// "bugs", "the test failed" no longer matched their root ("crash", "bug",
// "fail"), confirmed via a real-corpus regression. An unrelated word that
// merely shares a prefix ("call" inside "recall") still does not match,
// since only a recognized inflectional suffix (or doubled-consonant -ing/-ed)
// counts, not an arbitrary shared prefix.
const INFLECTION_SUFFIXES = new Set(['ing', 'ers', 'ions', 'ors', 'ies', 'es', 'ed', 'er', 'or', 'ly', 's']);
function stemsMatch(a, b) {
  if (a === b) return true;
  const short = a.length <= b.length ? a : b;
  const long_ = a.length <= b.length ? b : a;
  if (long_.startsWith(short) && INFLECTION_SUFFIXES.has(long_.slice(short.length))) {
    return true;
  }
  if (short && long_.startsWith(short + short[short.length - 1])) {
    const rest = long_.slice(short.length + 1);
    if (rest === 'ing' || rest === 'ed') return true;
  }
  return false;
}
function rawWords(text) {
  // Unlike tokenize() (used for bag-of-words TF-IDF, where dropping 1-letter
  // noise is harmless), this feeds the CONTIGUOUS/positional phrase matcher
  // below -- dropping a single-character word (e.g. "i") on both sides
  // silently closes the gap between adjacent words, collapsing a longer
  // pattern into a shorter one already in the same list. Confirmed: "i was
  // wrong" -> ["was","wrong"] was byte-identical to the separately-listed
  // "was wrong" pattern, so both fired and stacked their boost.
  return text
    .toLowerCase()
    .replace(/[^a-z0-9\s-]/g, ' ')
    .split(/\s+/)
...
// Full-phrase match: requires a CONTIGUOUS, in-order run of query words where
// each word stem-matches the corresponding phrase word -- preserves the
// original exact-substring-match semantics (word order and adjacency still
// matter), while tolerating inflection at each position ("crashed" matches a
// phrase word "crash"). An earlier version of this fix used an order-
// independent "all significant words present somewhere" check instead; that
// broke on short phrases dominated by stopwords ("was wrong", "that was
// wrong", "i was wrong" all reduce to the single word "wrong" once stopwords
// are stripped, so all three spuriously "matched" -- and stacked their boost
// three times over -- on any query merely containing the word "wrong"
// anywhere, e.g. "failure autopsy what went wrong"). Requiring the phrase's
// words verbatim, in place, adjacent to each other (only inflection-
// tolerant) avoids that collapse without needing special-case stopword
// handling: `queryWordsOrdered` is the RAW, unfiltered, order-preserving
// token list, not the deduped/stopword-filtered set used elsewhere below.
function phraseMatchesQuery(phraseText, queryWordsOrdered) {
  const phraseWords = rawWords(phraseText);
  if (phraseWords.length === 0) return false;
  for (let i = 0; i + phraseWords.length <= queryWordsOrdered.length; i++) {
    if (phraseWords.every((w, j) => stemsMatch(queryWordsOrdered[i + j], w))) return true;
  }
  return false;
}
// Partial/bag-of-words match: order-independent overlap count, matching the
// original partial-trigger-match design (never required contiguity, even
// before this fix) -- used only for the "how many words overlap" fallback
// and single-word checks (name-word boost, description-keyword boost), not
// full-phrase matching.
function phraseWordInQuery(phraseWord, queryWordsRaw) {
  return queryWordsRaw.some(qw => stemsMatch(qw, phraseWord));
}
function phraseWordMatchCount(phraseText, queryWordsRaw) {
  return rawWords(phraseText).filter(pw => phraseWordInQuery(pw, queryWordsRaw)).length;
}

// Public API: takes a raw query STRING (preserved for backward compat --
// this function is exported and called directly, with a string, elsewhere).
function buildIntentBoosts(queryLower) {
  const queryWordsOrdered = rawWords(queryLower);
  const intentBoosts = {};
  for (const intent of INTENT_PATTERNS) {
    const boost = intent.boost || BOOST_DEFAULT;
    for (const pattern of intent.patterns) {
      if (phraseMatchesQuery(pattern, queryWordsOrdered)) {
...
function assertUniqueSkillNames(skills) {
  // Warn (not throw) — consistent with validateIntentPatterns precedent.
  // Throwing here crashes matchSkills on every query when a user has two skills
  // with the same name (common during skill development/migration).
  const seen = new Set();
  ...
  if (duplicates.size > 0) {
    console.error(`[skill-router] WARNING: Duplicate skill names detected: ${[...duplicates].sort().join(', ')}`);
    console.error('[skill-router] First match wins. Remove duplicate skill directories to silence this warning.');
  }
}

/**
 * Applies heuristic score boosts (intent patterns, name/trigger/description
 * matches) and the anti_trigger hard veto to a set of scored results.
 * @returns {Array} A NEW array with vetoed skills removed. Always use the
 *   return value -- this does not always mutate `results` in place (a veto
 *   returns a filtered copy), so discarding the return value silently drops
 *   the veto. (A prior version of this function's callers discarded the
 *   return value, since the old code only ever mutated in place; that bug
 *   is why this contract is now spelled out here explicitly.)
 */
function applyHeuristicBoosts(results, skills, queryLower) {
  assertUniqueSkillNames(skills);
  // Two representations of the query, for the two matching modes above:
  // `queryWordsOrdered` (raw, unfiltered, order-preserving) for full-phrase
  // contiguous matching; `queryWordsRaw` (deduped, stopword-filtered) for
  // bag-of-words overlap counting. Dedup on the latter, outside the
  // per-skill loop below: a query with many repeated terms should not scan
  // the same term against every skill's description once per repetition
  // (defense-in-depth against a large/repetitive query -- the MCP layer
  // caps query length, but this keeps the per-call cost bounded by distinct
  // term count rather than raw token count either way).
  const queryWordsOrdered = rawWords(queryLower);
  const queryWordsRaw = [...new Set(queryWordsOrdered.filter(w => !STOP_WORDS.has(w)))];
  ...
  for (const result of results) {
    ...
    const nameWithSpaces = skill.name.toLowerCase().replace(/-/g, ' ');
    if (phraseMatchesQuery(nameWithSpaces, queryWordsOrdered)) {
      result.score += 0.5;
    }
    ...
    if (skill.triggers) {
      ...
      for (const trigger of skill.triggers) {
        if (phraseMatchesQuery(trigger, queryWordsOrdered)) {
          // Full match - this is the best possible
          bestTriggerBoost = 1.2;
          break;
        }
        // Partial trigger match
        const triggerWords = rawWords(trigger);
        const matchCount = phraseWordMatchCount(trigger, queryWordsRaw);
    ...
    // Anti-trigger veto — a matched anti_trigger hard-excludes the skill from
    // results entirely, rather than only applying a score penalty. A soft
    // penalty can still let an explicitly-rejected skill surface when few
    // other candidates exist or the rest of its score is high; mirrors the
    // hard veto already applied to the bash/Python UserPromptSubmit hook.
    // Kept as a strict literal (word-boundary) match rather than the
    // stem-aware matching above -- a hard exclusion should favor precision
    // over recall.
    if (skill.anti_triggers) {
      for (const anti of skill.anti_triggers) {
        if (wordBoundaryIncludes(queryLower, anti.toLowerCase())) {
          vetoed.add(result.name);
          break;
        }
      }
    }
    ...
    for (const word of nameWords) {
      if (word.length > 3 && phraseWordInQuery(word, queryWordsRaw)) {
        result.score += 0.3;
      }
    }
    ...
    if (skill.description) {
      const descWordsRaw = rawWords(skill.description);
      for (const term of queryWordsRaw) {
        if (descWordsRaw.some(dw => stemsMatch(dw, term))) {
          result.score += 0.2;
        }
      }
    }
  ...
  return results.filter(r => !vetoed.has(r.name));
}
...
function matchSkillsTfIdf(query, skills, topN = 5) {
  ...
  const boosted = applyHeuristicBoosts(results, skills, queryLower);
...
function resolveCoordinationChain(startSkillName, skills) {
  ...
  function collectPredecessors(name) {
    ...
    for (const req of info.coord.requires) {
      ...
      // Matches collectSuccessors' handling of missing "enables" targets below:
      // a transitively-missing require (not startSkillName's own immediate
      // requires, already hard-blocked above) is surfaced as a warning rather
      // than silently dropped, so the returned chain doesn't look complete
      // when a link further up the dependency graph is actually missing.
      else explanation.push(`[WARN] Required skill "${req}" is not installed — skipping`);
    }
  }
...
function buildPipeline(targetCapability, skills, availableArtifacts = ['user-intent'], options = {}) {
      ...
      for (const artifact of missing) {
        ...
        if (!resolve(producer)) {
          if (comp.optional) {
            ...
            explanation.push(`    [ERROR] ${skillName} requires ${artifact} but producer ${producer.name} failed to resolve`);
            return false;
          }
        }
      }
  ...
  // Start resolution from target skill. Its return value must be checked:
  // resolve() can legitimately exclude targetSkill itself from `pipeline`
  // (e.g. one of its dependencies failed to resolve) while still returning
  // normally -- without this check, buildPipeline would report error: null
  // and hand back an incomplete pipeline that never actually produces the
  // requested capability, with no signal to the caller that anything failed.
  const targetResolved = resolve(targetSkill);
  ...
  return {
    ...
    produced,
    explanation,
    error: targetResolved ? null : 'TARGET_RESOLUTION_FAILED'
  };
}
