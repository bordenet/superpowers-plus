...
const INTENT_PATTERNS = [
  ...
  { patterns: ['that was wrong', 'i was wrong', 'misdiagnosed', 'incorrect assumption',
               'wrong approach', 'wasted time', 'failed approach', 'post-mortem',
               'what went wrong', 'why did that fail'],
    skills: ['failure-autopsy'], boost: 4 },
...
function matchSkillsTfIdf(query, skills, topN = 5) {
  ...
  // Apply anti_trigger penalties — demote skills whose anti_triggers match the query
  for (const result of results) {
    const skill = skills.find(s => s.name === result.name);
    if (skill && skill.anti_triggers && skill.anti_triggers.length > 0) {
      for (const anti of skill.anti_triggers) {
        if (queryLower.includes(anti.toLowerCase())) {
          result.score *= 0.1; // Heavy penalty — demote but don't hide
          break;
        }
      }
    }
