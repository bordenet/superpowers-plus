...
// Intent boost scale — higher values indicate stronger signal that the skill applies.
// Scale is intentionally non-linear: most intents use BOOST_STANDARD (3).
const BOOST_DEFAULT   = 1.5; // No boost specified — weak signal
const BOOST_LOW       = 2;   // Generic code-change intents
const BOOST_STANDARD  = 3;   // Domain-specific intent clearly maps to a skill
const BOOST_ELEVATED  = 4;   // Multi-step workflow skills (evolution-loop, micro-review)
const BOOST_HIGH      = 5;   // Code review variants — must outrank generic matches
const BOOST_CRITICAL  = 6;   // Reviewer protocol — active review session
const BOOST_EMERGENCY = 8;   // Failure autopsy — user explicitly describing a disaster
...
const INTENT_PATTERNS = [
  ...
  // Wiki markdown structure intents (before generic wiki — catches malformed tables/fences)
  ...
  { patterns: ['compare design approaches', 'design comparison matrix', 'evaluate design alternatives',
    ...
    skills: ['design-triad'], boost: BOOST_STANDARD },
  ...
  { patterns: ['request code review', 'submit for review', 'need review',
    ...
    skills: ['code-review'], boost: BOOST_HIGH },
  ...
  { patterns: ['code change', 'modify code', 'write code', 'update code', 'change code',
    ...
    skills: ['feature-development', 'brainstorming', 'design-triad'], boost: BOOST_LOW },
