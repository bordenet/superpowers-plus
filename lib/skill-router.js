...
function applyHeuristicBoosts(results, skills, queryLower, rawTerms) {
  ...
  // Dedup once, outside the per-skill loop below: a query with many repeated
  // terms should not scan the same term against every skill's description
  // once per repetition (defense-in-depth against a large/repetitive query --
  // the MCP layer caps query length, but this keeps the per-call cost bounded
  // by distinct term count rather than raw token count either way).
  const uniqueRawTerms = [...new Set(rawTerms)];
