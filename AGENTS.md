...
- **Pre-push runs `test-all.sh --fast` automatically** (Gate 1 of 6) — a push can no longer go out without the local suite passing first; this is not a step to remember, it's just what pushing does. Gate 6 requires `.llm-skill-review-cleared` for `skills/*.md` changes; it supersedes (not supplements) the code-review and PHR gates for that file class specifically.
...
- **Writing a NEW hook or gate that invokes nested git directly (not a bats fixture)?** `tools/pre-push`'s own `unset` of the same four vars (near its top) exists for a separate, still-live vector: git auto-sets `GIT_DIR` for hook subprocesses under a linked worktree, which silently overrides `-C`/cwd discovery for any nested git command the hook runs — sanitize the same four vars before your own nested git calls.
