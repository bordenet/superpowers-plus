...
check '\${HOME} form'                               FOUND 'export PATH="${HOME}/.local/bin:$PATH"'
check 'tilde form (unquoted, expands)'              FOUND 'export PATH=~/.local/bin:$PATH'
...
check 'single-quoted \$HOME shorthand (no expand)'  NOT_FOUND "export PATH='\$HOME/.local/bin:\$PATH'"
check 'single-quoted \${HOME} shorthand (no expand)' NOT_FOUND "export PATH='\${HOME}/.local/bin:\$PATH'"
check 'single-quoted tilde shorthand (no expand)'   NOT_FOUND "export PATH='~/.local/bin:\$PATH'"
check 'double-quoted tilde shorthand (no expand)'   NOT_FOUND 'export PATH="~/.local/bin:$PATH"'
check 'tilde mid double-quoted token (no expand)'   NOT_FOUND 'export PATH="$PATH:~/.local/bin"'
check 'tilde after \${..} inside dquote (no expand)' NOT_FOUND 'export PATH="${PATH:+$PATH:}~/.local/bin"'
check 'dquote-span then tilde (span abuts tilde)'   NOT_FOUND 'export PATH="${PATH:+$PATH:}"~/.local/bin'
check 'tilde glued to dquote suffix'                NOT_FOUND 'export PATH=~/.local/bin"$PATH"'
check 'tilde colon-sep with dquote suffix'          NOT_FOUND 'export PATH="$PATH":~/.local/bin"$PATH"'
check 'PATH+= tilde double-quoted (no expand)'      NOT_FOUND 'PATH+="~/.local/bin:"'
