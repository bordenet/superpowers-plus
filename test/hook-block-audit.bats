...
@test "red-autonomy block logs TP class and sanitized session id without command text" {
  ...
  [[ "$log" == *"red-autonomy exit=2 reason=no-approval class=TP sid=redsession42"* ]]
  [[ "$log" != *"private-branch-name"* ]]
}

@test "internal-terms block logs TP class and session id without matched content" {
  ...
  [ "$status" -eq 2 ]
  log="$(cat "$TEST_HOME/.claude/hooks/hook-audit.log")"
  [[ "$log" == *"internal-terms exit=2 reason=hits-found class=TP sid=terms-session"* ]]
  [[ "$log" != *"private-block-marker"* ]]
}

@test "git-identity block logs TP class and session id without identity values" {
  local payload log
  make_repo
  write_identity_rules
  payload="$(jq -cn \
  ...
  [ "$status" -eq 2 ]
  log="$(cat "$TEST_HOME/.claude/hooks/hook-audit.log")"
  [[ "$log" == *"git-identity exit=2 reason=mismatch class=TP sid=identity-session"* ]]
  [[ "$log" != *"private-message"* ]]
...
@test "hook block report counts real malformed hook exits end to end" {
  ...
  expected=$'hook\texit\tTP\tFP\tunknown\nred-autonomy\t2\t0\t0\t1\ninternal-terms\t2\t0\t0\t0\ninternal-terms\t5\t0\t0\t1\ngit-identity\t2\t0\t0\t0\ngit-identity\t5\t0\t0\t1\nTOTAL\t-\t0\t0\t3\nignored_lines\t0\ntruncated\tno'
...
EOF
  expected=$'hook\texit\tTP\tFP\tunknown\nred-autonomy\t2\t1\t0\t0\ninternal-terms\t2\t0\t1\t0\ninternal-terms\t5\t0\t0\t1\ngit-identity\t2\t0\t0\t1\nTOTAL\t-\t1\t1\t2\nignored_lines\t7\ntruncated\tno'
...
@test "hook block report reads only the requested tail of a large log" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *$'red-autonomy\t2\t0\t0\t0'* ]]
  [[ "$output" == *$'internal-terms\t2\t0\t1\t0'* ]]
  [[ "$output" == *$'TOTAL\t-\t0\t1\t0'* ]]
...
@test "hook block report retains a complete record at the tail boundary" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *$'red-autonomy\t2\t1\t0\t0'* ]]
  [[ "$output" == *$'git-identity\t2\t1\t0\t0'* ]]
  [[ "$output" == *$'TOTAL\t-\t2\t0\t0'* ]]
  [[ "$output" == *$'truncated\tyes'* ]]
}
