#!/usr/bin/env bats
# Tests for the pre-push Gate 3 (.merge-discipline-cleared consumer side).
# Extracts the check_merge_discipline_sentinel function from tools/pre-push
...
NC=$'\033[0m'
EOF
    # Extract just the check_merge_discipline_sentinel function from pre-push.
    awk '/^check_merge_discipline_sentinel\(\)/,/^}$/' "$REPO_ROOT/tools/pre-push" \
        > extracted-fn.sh
    ...
    mkdir -p tools
    : > tools/merge-discipline-preflight.sh
    chmod +x tools/merge-discipline-preflight.sh
    cat > harness.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
source ./stub-colors.sh
REPO_ROOT="$PWD"
MERGE_DISCIPLINE_SENTINEL="$PWD/.merge-discipline-cleared"
EOF
    cat extracted-fn.sh >> harness.sh
    cat >> harness.sh <<'EOF'

# Args: $1=target $2=pushed_sha
check_merge_discipline_sentinel "$1" "$2"
EOF
    chmod +x harness.sh
}
...
@test "gate3: valid sentinel for dev with matching SHA -> PASS, sentinel consumed" {
    echo "v1|abc123|feature/foo|dev|2026-05-24T00:00:00Z" > .merge-discipline-cleared
    run ./harness.sh dev abc123
    [ "$status" -eq 0 ]
    [[ "$output" == *"merge-discipline cleared"* ]]
    [ ! -f .merge-discipline-cleared ]  # consumed after PASS
}

@test "gate3: valid sentinel for staging -> PASS" {
    echo "v1|def456|dev|staging|2026-05-24T00:00:00Z" > .merge-discipline-cleared
...
@test "gate3: valid sentinel for main -> PASS" {
    echo "v1|789aaa|staging|main|2026-05-24T00:00:00Z" > .merge-discipline-cleared
...
@test "gate3: missing sentinel for dev push -> FAIL" {
    rm -f .merge-discipline-cleared
    run ./harness.sh dev abc123
    [ "$status" -ne 0 ]
    [[ "$output" == *"No .merge-discipline-cleared sentinel"* ]]
}

@test "gate3: sentinel with wrong target -> FAIL" {
    echo "v1|abc123|dev|staging|2026-05-24T00:00:00Z" > .merge-discipline-cleared
    run ./harness.sh dev abc123  # sentinel says staging, pushing to dev
    [ "$status" -ne 0 ]
    [[ "$output" == *"target"* ]] && [[ "$output" == *"!="* ]]
    [ -f .merge-discipline-cleared ]  # NOT consumed on failure
}
...
@test "gate3: sentinel with bad version -> FAIL" {
    echo "v0|abc123|feature/foo|dev|2026-05-24T00:00:00Z" > .merge-discipline-cleared
    run ./harness.sh dev abc123
    [ "$status" -ne 0 ]
...
@test "gate3: sentinel with only 4 fields -> FAIL" {
    echo "v1|abc123|feature/foo|dev" > .merge-discipline-cleared
    run ./harness.sh dev abc123
    [ "$status" -ne 0 ]
    [[ "$output" == *"format unrecognized"* ]]
    [ -f .merge-discipline-cleared ]
...
@test "gate3: sentinel with empty SHA field -> FAIL" {
    echo "v1||feature/foo|dev|2026-05-24T00:00:00Z" > .merge-discipline-cleared
    run ./harness.sh dev abc123
    [ "$status" -ne 0 ]
    [[ "$output" == *"format unrecognized"* ]]
    [ -f .merge-discipline-cleared ]
...
@test "gate3: sentinel with empty source field -> FAIL" {
    echo "v1|abc123||dev|2026-05-24T00:00:00Z" > .merge-discipline-cleared
    run ./harness.sh dev abc123
    [ "$status" -ne 0 ]
    [[ "$output" == *"format unrecognized"* ]]
    [ -f .merge-discipline-cleared ]
...
@test "gate3: sentinel with empty target field -> FAIL" {
    echo "v1|abc123|feature/foo||2026-05-24T00:00:00Z" > .merge-discipline-cleared
    run ./harness.sh dev abc123
    [ "$status" -ne 0 ]
    [[ "$output" == *"format unrecognized"* ]]
    [ -f .merge-discipline-cleared ]
