...
| "Failure mode" / "failure class" / "failure pattern" / "failure category" / "error class" / "defect pattern" (singular and plural) | Do not replace structural uses, external quotations, code/API documentation describing a class/type construct, or a concrete singular use that names how the system fails (a process lapse does not count). Free-running prose: apply the decision tree from reference.md "AI Jargon Replacement Guide" section. Default when uncertain: replace with the named problem ("the bug was X", "what typically goes wrong is Y"). |
