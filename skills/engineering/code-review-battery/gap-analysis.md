...
**candidate-011** (graduated 2026-07-24, live-incident): Gate TOCTOU -- Local State Used to Gate a Remote Action. Source gap: agent-factory MR !3 -- `glab-mr-ready.sh` checked a local `.code-review-cleared-<SHA>` sentinel file, then called `glab mr update --ready`. A CI bot commit between sentinel-write and the `mr-ready` call advanced the remote HEAD, so the script marked an unreviewed SHA ready. Defect Finder's Concurrency dimension lists TOCTOU but did not enumerate the gating shape (local file check -> remote API call) as a concrete trigger. Graduated into `defect-finder.md` "Gate TOCTOU" section. Fix: fetch authoritative state from remote API immediately before the gate check; reject if local sentinel SHA does not match API-reported HEAD SHA. Confidence 0.8 -- live incident confirms the failure mode; single exercise only, no holdout separation.

**candidate-012** (graduated 2026-07-24, live-incident): API Response Type Filtering Before Aggregation. Source gap: agent-factory MR !3 -- `glab api search?scope=notes` returns heterogeneous `noteable_type` values (MergeRequest, Issue, Snippet). Code deduped on `(project_id, noteable_iid)` without first filtering to MergeRequest, folding issue timestamps into MR candidate dedup. Defect Finder's Edge Cases dimension covers API failures but not heterogeneous-type API results. Graduated into `defect-finder.md` "API Response Type Filtering" section. Confidence 0.8 -- live incident; single exercise, no holdout.

**candidate-013** (graduated 2026-07-24, live-incident): API Pagination Completeness. Source gap: agent-factory MR !3 -- `glab api` without `--paginate` returns only the first page of results (default 20 items). The watchdog search silently missed all recently-triggered MR notes beyond page 1. ShellRuntimeAuditor's Tool Contract Safety dimension covered flag-parsing completeness and exit codes but did not enumerate the pagination flag as a concrete tool contract to verify. Graduated into `shell-runtime-auditor.md` "API pagination completeness" check in Dimension 3 (Failure-Mode Resilience) rather than Dimension 2 (Tool Contract Safety) because the user-visible failure mode is silent data loss -- a resilience failure -- even though the root cause is a tool-contract gap; the tool-contract framing explains the root cause, not the severity category. Confidence 0.8 -- live incident; single exercise, no holdout.

**candidate-014** (graduated 2026-07-24, live-incident): Bats Test Isolation (BATS_TMPDIR). Source gap: agent-factory MR !3 -- bats tests that `cd` into the real checkout and invoke the sentinel-write script without `setup()`/BATS_TMPDIR isolation can write `.code-review-cleared-*` files into the live repo. ShellRuntimeAuditor did not check bats-specific isolation patterns. Graduated into `shell-runtime-auditor.md` "Test isolation for bats suites" check in Dimension 3. Also documents that `run cd <real-repo>` is safe -- it executes in a subshell and does NOT propagate CWD to the outer test function, clarifying a prior misconception; only a bare `cd` outside `run` is a risk. Confidence 0.8 -- live incident; single exercise, no holdout.
