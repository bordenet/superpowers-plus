...
| Duplicated on-disk format parsing (config/token/cache file minted by one script, read via ad-hoc `cat`/`sed`/`awk`/`grep` by others) | Grep every consumer of the file path/pattern repo-wide, tests included; prefer extracting a shared parser over updating each consumer in place |

## Diff-Size-Scaled Dispatch Exemption

**Reduced dispatch for a diff the reviewing agent already has current, verified context on (not self-attested):** parallel sub-agent dispatch (Phase 2) exists to compensate for a reviewer with NO conversation context reading a diff cold. When the SAME agent that just wrote (or just fully re-read) the exact code under review is doing the reviewing, and the diff is small and low-risk, dispatching 4-7 fresh sub-agents to independently re-derive context the reviewing agent already holds is waste, not rigor. This exemption changes DISPATCH MODALITY only (parallel sub-agents -> a single inline self-review pass by the reviewing agent); it does NOT relax Phase 6's evidence bar -- a genuine evidence block per finding/clean-dimension, with real re-runnable commands, is still mandatory, and the score/verdict math is unchanged.

**Trigger conditions (ALL must hold):**
- Diff is small and structurally simple: a useful ballpark is <=150 changed lines across <=3 files -- well under the Large-diff pre-dispatch gate's 3000-line/60KB chunking threshold at the other extreme -- but the deciding factor is the next bullet, not the line count alone.
- The reviewing agent has CURRENT, VERIFIED context on the exact code under review from this same session (wrote it, or fully re-read the post-change file in this same pass) -- not recollection from an earlier turn, not an assumption about what the code probably does.
- Not in BugPath Mode (Phase 0.5) -- a confirmed bug-fix branch always gets full dispatch (BugPath Verifier mandatory), regardless of diff size.
- **No Mandatory-activation or Signal-driven-dispatch trigger fires** (Phase 1 tables) -- if Guardian's always-on triggers (retry/circuit-breaker/rollback/deployment-config/feature-flag/auth/state-machine) or ANY signal-driven row (security-class signal, metric/alarm definition, public API/schema change, file rename/move/delete needing inbound-reference scan, etc.) match, this exemption does not apply -- dispatch normally regardless of size. Mirrors the cited-port exemption's own carve-out logic: this reduces dispatch OVERHEAD, never dispatch SUBSTANCE for a risk class that's actually present.

**What the inline pass must still do:** explicitly answer each activated reviewer's core question set as if that persona had been dispatched (Defect Finder: correctness/edge cases/concurrency; Guardian: security/blast-radius/backwards-compat; Standards Enforcer: docs/test quality/observability; plus any Phase 1 signal-driven reviewer whose trigger condition is present) -- not skip the substance, only the overhead of separate sub-agent dispatch and cross-agent aggregation. State the triage line as `**Triage**: Inline Review (single-pass, not independently-dispatched personas) | Reason: [1-2 sentences citing which trigger conditions above were confirmed]` so a reader can audit the exemption claim itself, the same way the cited-port exemption requires a re-runnable citation rather than a bare assertion.

**No evidence in the triage line for why this diff qualifies means unverified -- dispatch normally.** (Example from production use: a one-function `resolve_base()` fallback-chain fix in `tools/md-files-changed.sh`, ~15 changed lines, no security/API/state-machine signal, reviewed inline by the same agent that had just finished tracing the bug -- triage line: "Inline Review (single-pass...) -- small, well-understood fix mirroring an already-validated pattern from the same session's pre-push refactor.")
