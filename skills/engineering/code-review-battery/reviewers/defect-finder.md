...
5. Every finding from this pattern is a reachability claim -- it MUST comply with Guardian's "Anti-Hallucination Gate: Reachability Claims" evidence format (`guardian.md`) when reporting: a `Reachability evidence:` field stating either `Found:` (quote the call site/reference that resolves it) or `Not found:` (list every file/excerpt scanned). A "no remaining callers" claim without that field is unverifiable, not confirmed.
   - **Mechanized evidence applies ONLY to the single-declaration, no-dynamic-dispatch case -- everything else falls back to prose, at reduced severity.** Before choosing the JSON `evidence.command` path below, positively check: (a) exactly ONE declaration/re-export site for the symbol (`grep -rn "export.*{[^}]*\bsymbolName\b[^}]*}.*from"` across the repo -- 2+ hits, or any hit outside the file you think is "the" declaration, means this does NOT qualify), and (b) no plausible dynamic dispatch per step 3 above. **If either check fails, or you are unsure, do NOT attempt the mechanized command below** -- file the finding using Guardian's prose-only `Reachability evidence: Not found: <files scanned>` format instead, mark it `"verifiable": false` (so the existing, tested `UNVERIFIABLE_CAP` path applies, not a silent skip), AND cap the severity at **Possible** regardless of exported/published-library status -- a repo-scoped grep this mechanized checker itself can't safely handle (multi-declaration, barrel re-exports, string-keyed registries) shouldn't be trusted at full Important/Critical severity either. This is a deliberate, permanent exemption for this specific subclass, not a placeholder to "fix properly later."
