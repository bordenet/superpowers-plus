...
5. Every finding from this pattern is a reachability claim -- it MUST comply with Guardian's "Anti-Hallucination Gate: Reachability Claims" evidence format (`guardian.md`) when reporting: a `Reachability evidence:` field stating either `Found:` (quote the call site/reference that resolves it) or `Not found:` (list every file/excerpt scanned). A "no remaining callers" claim without that field is unverifiable, not confirmed.
   - **No mechanized JSON evidence for this pattern -- prose-only, always.** File every Caller Removal Trace finding with Guardian's prose `Reachability evidence: Not found: <files scanned>` format, `"dimension": "Correctness"`, and `"evidence": {"verifiable": false, "rationale": "..."}` (no `command`/`expectation` fields) -- routing through the existing, already-tested `UNVERIFIABLE_CAP = 7.0` path, not a mechanized replay. This is a deliberate, permanent decision, not a placeholder: three review rounds fixing a mechanized grep-replay idiom for this pattern (word-boundary anchoring, exit_code-vs-count semantics, comment-exclusion, multi-declaration/re-export exclusion, symlinked-directory traversal, case-insensitive-filesystem git-checkout collisions) found a new bypass every single round -- proving global absence of a caller across an arbitrary repo's filesystem via grep-replay does not converge. **Known consequence, not a defect:** because the verifier's cap is keyed on `(reviewer, dimension)` and takes the minimum across all claims sharing that tuple, an unconditionally-unverifiable Caller Removal Trace finding caps EVERY other Defect Finder "Correctness" finding in the same run at 7.0 too, even ones with a clean, fully-mechanized `evidence.command` of their own -- this is accepted, not accidental: it forces a human-reviewed round whenever this pattern fires a real finding, which is the intended effect, not a scoring bug to route around.
...
### Sibling Path Trace (Parallel-Handler Consistency)

Some defects are invisible from the diff alone because the diff is only half of a change that should have touched a structurally-parallel sibling -- a create/update/delete triad for the same resource, a reschedule/cancel/create variant of the same workflow, a sync/async twin of the same operation. The diff is internally consistent and passes tests; it's asymmetric relative to a sibling path that nothing in the diff's own text points to. This is the harder, cross-file cousin of Consumer Trace above -- Consumer Trace finds readers of a field that exists; this finds readers that SHOULD exist but weren't added.

Trigger conditions (either is sufficient):

- The diff adds a new property to a type/interface/shared-state object that is already imported or consumed by 2+ non-test files elsewhere in the repo (candidate check only -- confirmed by the "When triggered" procedure below, not fired on this bullet alone).
- The diff modifies some but not all files among a family of 2+ files in the same directory that share a common stem or a create/update/post vs. reschedule/cancel/delete naming pattern for the same resource (e.g. `post-x.ts` + `reschedule-x.ts` + `cancel-x.ts` all handling `x`), where the change supplements or partially supersedes a field those siblings already read -- this includes the common 2-member case (one touched, one not) but is not limited to it: a family of 4 siblings with 2 touched and 2 untouched triggers the same check against each untouched member.

When triggered:

1. Identify the semantic decision the new/changed field makes (e.g., "which record to act on").
2. Ask whether an EXISTING field already makes a related or overlapping decision (often named similarly, or read at the same call site the new field now also feeds). If the new field supplements or partially supersedes an existing one, find every current reader of that existing field across the repo -- not just readers of the new field, which may not exist yet.
3. For each such reader NOT touched by this diff: does the new field's logic need to apply there too? If the sibling reader still only consults the old field/logic, and the new field was added specifically because the old logic was insufficient, the sibling has the same defect the diff was written to fix.
4. Flag any untouched sibling as **Important** (or **Critical** if it reproduces a confirmed production defect the diff was meant to close).
5. This pattern makes a claim analogous to a reachability claim -- "no other reader needed updating" -- and is held to the same evidentiary bar as Guardian's "Anti-Hallucination Gate: Reachability Claims" (`reviewers/guardian.md`): every finding, AND every explicit "no sibling gap found" determination, MUST use the literal field name `Reachability evidence:` with Guardian's own `Found:`/`Not resolved:`/`Not found:` vocabulary, not an ad-hoc substitute -- `Found:` quoting the sibling's own still-live read of the pre-existing field (this IS the gap), or `Not found:` naming every sibling file actually opened and confirming none still needs the update. A "no gap" conclusion that doesn't name the sibling files actually opened is unverifiable, not confirmed -- the expensive part of this pattern (opening every sibling file) is exactly the part a time-pressured reviewer is tempted to skip while still asserting the negative. **No mechanized JSON evidence for this pattern either -- prose-only, always**, for the same reason as Caller Removal Trace above: proving no other reader needs updating is a repo-wide absence claim a grep-replay idiom cannot durably verify (see step 4's `verifiable: false` / `UNVERIFIABLE_CAP = 7.0` note); route Sibling Path Trace findings through the identical path.
...
- **Severity** (use these definitions consistently):
  ...
  - **Possible**: a plausible-but-unconfirmed finding (e.g. a plausible dynamic caller for Caller Removal Trace, an indirect producer for Producer Trace), used only as an explicit downgrade from Critical/Important/Minor. Never assigned directly or elevated; informational only, excluded from the score formula.
