...
- The diff adds a new property to a type/interface/shared-state object that is already imported or consumed by 2+ non-test files elsewhere in the repo.
...
5. This pattern makes a claim analogous to a reachability claim -- "no other reader needed updating" -- and is held to the same evidentiary bar as Guardian's "Anti-Hallucination Gate: Reachability Claims" (`reviewers/guardian.md`): every finding, AND every explicit "no sibling gap found" determination, MUST use the literal field name `Reachability evidence:` and name which sibling file(s) were actually opened and what they were found to read/consult (e.g. "checked dispatch-batch.ts: reads job.routingKey only; comment confirms per-job overrides are explicitly out of scope for batch dispatch, by design"). A "no gap" conclusion that doesn't name the sibling files actually opened is unverifiable, not confirmed -- the expensive part of this pattern (opening every sibling file) is exactly the part a time-pressured reviewer is tempted to skip while still asserting the negative.

**Example**: A diff adds `preferredSlotOwner` (a new, more accurate assignment field) and updates the CREATE handler to read it, falling back to the old `preferredOwner` field only when the new map has no entry. The RESCHEDULE handler -- a sibling file in the same directory, handling the same underlying resource -- still only reads `preferredOwner` and was never touched by the diff. When `preferredOwner` is cleared upstream (because the original owner turned out to be unavailable), reschedule falls back to a stale, already-confirmed-unavailable owner instead of the new field's accurate answer -- the exact defect the diff was written to close, just on the untouched sibling path.
