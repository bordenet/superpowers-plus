...
5. Every finding from this pattern is a reachability claim -- it MUST comply with Guardian's "Anti-Hallucination Gate: Reachability Claims" evidence format (`guardian.md`) when reporting: a `Reachability evidence:` field stating either `Found:` (quote the call site/reference that resolves it) or `Not found:` (list every file/excerpt scanned). A "no remaining callers" claim without that field is unverifiable, not confirmed.
   - **Mechanized evidence applies ONLY to the single-declaration, no-dynamic-dispatch, no-symlinked-path case -- everything else falls back to prose, at reduced severity.** Before choosing the JSON `evidence.command` path below, positively check: (a) exactly ONE declaration/re-export site for the symbol (`grep -rn "export.*{[^}]*\bsymbolName\b[^}]*}.*from"` across the repo -- 2+ hits, or any hit outside the file you think is "the" declaration, means this does NOT qualify); (b) no plausible dynamic dispatch per step 3 above; and (c) **run `find <repo-root> -type l` and confirm none of the candidate caller files sit under a symlinked directory** -- plain `grep -r` does NOT follow symlinked directories by default, so a real caller reachable only through a symlink (common in monorepo workspace-link setups -- pnpm/Lerna/Bazel-style shared/vendor directories) is invisible to the mechanized command even when (a) and (b) both correctly pass; treat ANY symlink found in the repo tree as disqualifying unless you can positively confirm none of the candidate files are reachable through it. **If any of (a)/(b)/(c) fails, or you are unsure, do NOT attempt the mechanized command below** -- file the finding using Guardian's prose-only `Reachability evidence: Not found: <files scanned>` format instead, mark it `"verifiable": false` (so the existing, tested `UNVERIFIABLE_CAP` path applies, not a silent skip), AND cap the severity at **Possible** regardless of exported/published-library status -- a repo-scoped grep this mechanized checker itself can't safely handle (multi-declaration, barrel re-exports, string-keyed registries, symlinked workspace links) shouldn't be trusted at full Important/Critical severity either. This is a deliberate, permanent exemption for this specific subclass, not a placeholder to "fix properly later."
   - **For the qualifying single-declaration case**, the declaration's own file is not evidence of a caller -- exclude it from the grep target, don't rely on the verifier to distinguish declaration from call site. A bare `grep -rlE '\bsymbolName\b' .` over the WHOLE repo will match the declaration line itself, and a naive `expectation: {type: "count", value: "==0"}` piped through `wc -l` self-falsifies (the verifier's `count` type checks the command's own output LINE COUNT, not a number the command prints -- `grep -l ... | wc -l` always prints exactly one line, so `count` sees `non_blank_lines=1` regardless of the actual number). List the OTHER files that could plausibly call it (excluding the declaration's own file) and use `test $(grep -rlE '\bsymbolName\b' file1 file2 ... 2>/dev/null | wc -l) -eq 0` with `expectation: {type: "exit_code", value: 0}` -- the same idiom this file already uses elsewhere for absence claims. Verified example: `{"claim": "retryWithBackoff() has zero remaining call sites outside its own declaration", "evidence": {"command": "test $(grep -rlE '\\bretryWithBackoff\\b' src/worker.ts src/sync-job.ts src/webhook-sender.ts 2>/dev/null | wc -l) -eq 0", "expectation": {"type": "exit_code", "value": 0}, "verifiable": true, "rationale": "checks every file that could plausibly call it, excluding retry-helpers.ts (the declaration site itself)"}}`. Note the doubled backslash in `\\bretryWithBackoff\\b` -- see the JSON-escaping warning under Forbidden Command Patterns below; this is not optional. **Comment-only mentions still self-falsify this idiom** (a `//`/`/** */` mention of the symbol matches the grep even though step 1 says it's not a real reference) -- if the ONLY remaining hits after this check are comment-only, do not trust the mechanized exit_code alone: quote the comment-only lines in the finding's prose and note explicitly that they were excluded as non-references, same as the multi-declaration fallback above.
