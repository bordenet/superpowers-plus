...
- **Catalog-vs-code consistency** (MANDATORY when the diff modifies a metric catalog entry OR adds/changes an emit site): Read the catalog description (the metric's `defineMetric` description string + any inline doc-comment within 10 lines) as a BOOLEAN ASSERTION about WHICH PATHS emit the metric. Then trace every emit site AND every fail-open / catch / finally / early-return path. If the description says "NOT emitted on X" and the code emits on X (or vice versa), that is a **Critical** finding — operator dashboards and alarms read the catalog as truth. A 1-line fix in the wrong place (failing to null a timing variable in a catch block; missing `if (this.hasEnded) return` before an emit) silently corrupts the histogram for every fail-open call. This rule generalizes Comment-as-Spec from inline comments to metric-catalog descriptions. (Real miss: incident-2026-1507 -- catalog said "excludes fail-open" but the catch path didn't null the timestamp before the finally re-emitted.)
