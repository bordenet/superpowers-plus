...
description: "Use when reviewing code changes to dispatch parallel specialized reviewers instead of a single monolithic review — provides deeper, more precise findings across 5 focused lenses. Invoke as: /sp-cr-battery [min-score] (optional 1.0–10.0 quality threshold, default 7.0)"
...
**Signal-driven dispatch** (additive — activates reviewer(s), never deactivates; skip any reviewer with nothing in scope and say why in the triage line). Observability routes: a metric/event definition or `.emit(`/`.inc(`/`publish(` call, or an alarm/threshold definition or error branch that feeds an alarm → **Defect Finder + Standards Enforcer + Guardian** (Producer Trace, metric liveness, emission symmetry, alarm-feeding isolation, failure-mode differentiation); an external-dependency call or SDK error classifier (429/5xx/provider shapes) → **Guardian** (multi-provider predicate coverage, monitoring blast radius).
...
- `run_timestamp` -- ISO 8601 UTC, minted by the orchestrator at write time
- `head_sha` -- `git rev-parse HEAD`
- `verdict`, `score`, `rounds` -- from the verdict determination above
- `findings: [...]` -- each entry serializes the Phase 3 fields: `reviewer`, `severity`, `file`, `line`, `issue`, `regressions_risked`, `durable_check`, `classification`

`tools/run-battery.sh` refuses to write the sentinel if `.cr-battery-runs/` exists but the per-HEAD JSON is missing or empty (graceful degradation: skipped if the directory is absent, or in `--staged` mode). If `jq` is available, the gate also validates that the file is parseable JSON. This is preservation only -- no verdict tagging, no shared corpus. See `docs/cr-battery/finding-lifecycle-design.md` for the deferred lifecycle work.
