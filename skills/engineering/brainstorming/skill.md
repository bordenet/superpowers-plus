...
4. **If ≥3 viable approaches emerged** — invoke `debate` to formally compare and red-team the already-surfaced approaches (NOT to generate new ones). `debate` expects to be handed this spec (or its inline summary) rather than re-deriving requirements itself — the written spec from step 1 is what it points to.
