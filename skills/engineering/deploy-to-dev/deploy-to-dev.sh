...
# Description: Deploy a [PRODUCT] service branch to the Dev environment via Azure Pipelines.
...
readonly PROJECT="[PRODUCT] Phone Assist"
...
Prerequisites:
  ...
  - Azure DevOps defaults configured (run: az devops configure --defaults project="[PRODUCT] Phone Assist")
EOF
  exit 1
}
