...
description: Deploy a CARI service branch or PR to the Dev environment without merging. Uses Azure Pipelines manual trigger to build, push Docker image, and update ECS. No Docker or AWS credentials required locally — only az CLI.
...
- Deploy a CARI branch or PR to Dev without merging to `main`
...
Deploy any CARI service branch to the Dev environment in ~8-10 minutes. No Docker, no AWS credentials, no PowerShell required. Just `az` CLI.
...
3. **Azure DevOps defaults set** — `az devops configure --defaults project="CARI Phone Assist" organization=https://dev.azure.com/CallBox`

To verify: `az pipelines list --project "CARI Phone Assist" --output table` should show pipelines.
...
|----------|-------|--------|
| **Thomas Smith** | `tsmith@callbox.com` | `7fa4220c-6614-6628-9991-54c7d1f85e40` |
| **Junyi Sim** | `jsim@callbox.com` | `7a5b2fd1-9491-6e66-b721-97413f508310` |
...
Any service with an Azure Pipeline in the CARI Phone Assist project:
...
| "No pipeline found" | Check service name matches a pipeline. Run `az pipelines list --project "CARI Phone Assist"` |
...
| "Deploy to Dev failed" | Check pipeline logs: `az pipelines runs show --id <RUN_ID> --project "CARI Phone Assist"` |
