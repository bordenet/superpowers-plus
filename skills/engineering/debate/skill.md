...
**Before picking route 1 or 2 on a non-trivial decision, confirm the requirements/architecture summary you're about to state didn't come from nowhere.** If `brainstorming` already ran for this problem, point to its written spec instead of re-deriving requirements from memory — reasoning that only ever existed inline (never written down) is vulnerable to being lost to context compaction with nothing to recover it from. If `brainstorming` never ran and this isn't route 3's low-stakes case, run it first rather than inventing requirements on the spot — this is a one-time bootstrap call, not the mid-investigation pattern route 2 forbids above: brainstorming's own process ends by optionally invoking `debate`, and on that second pass this same check finds the spec already written and points to it instead of recursing again.
