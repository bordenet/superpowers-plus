...
**Before picking route 1 or 2 on a non-trivial decision, confirm the requirements/architecture summary you're about to state didn't come from nowhere.** If `brainstorming` already ran for this problem, point to its output (the written spec, or its inline decision summary if the user explicitly skipped the file-write step) instead of re-deriving requirements from memory — reasoning that only ever existed inline is vulnerable to being lost to context compaction with nothing to recover it from. If `brainstorming` never ran and this isn't route 3's low-stakes case, run it first rather than inventing requirements on the spot.
