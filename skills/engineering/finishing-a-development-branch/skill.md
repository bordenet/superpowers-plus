...
Prefer the branch's own tracking upstream, then this repo's actual workflow
...
for candidate in "$tracking" origin/dev dev origin/staging staging origin/main main origin/master master; do
...
If any candidate returns a SHA, use that branch as the base. If all fail, the branch has no recognizable integration ancestor — surface this explicitly: "Could not determine base branch automatically (no tracking/dev/staging/main/master ancestor found). What branch did this work split from?"
...
- `--force`-removing a worktree that refused removal — see `references/worktree-cleanup.md`
