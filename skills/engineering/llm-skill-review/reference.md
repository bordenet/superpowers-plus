...
`tools/pre-push`'s Gate 6 (`tools/pre-push-llm-skill-review-gate.sh`) requires `.llm-skill-review-cleared` for any push touching `skills/*.md`, at the same 9.2 minimum score this repo already enforced for skill changes before this gate existed (moved here, not a new threshold). This **supersedes** the PHR gate and the code-review gate for that file class specifically -- both of those gates explicitly exclude `skills/*.md` from their own scope now, so a `skills/*.md`-only push requires exactly this one sentinel, not `.phr-cleared` and `.code-review-cleared` as well.
