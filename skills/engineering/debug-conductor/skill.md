...
3. **Classify hypothesis domains:**
   - Timeline / trace gaps, including telephony call-flow sequencing
   - LLM / prompt behavior
   - State consistency
   - Infrastructure / config / deployment
   - Reproducible hypotheses requiring a controlled experiment
...
1. **Select investigator prompts** based on classified domains (max 4), using these exact paths:

   | Domain | Reference path | Dispatch rule |
   |--------|----------------|---------------|
   | Timeline, trace, or telephony call-flow sequencing | `references/timeline-trace-investigator.md` | Include when trace, log, deployment, metric, or call-flow timing evidence is available |
   | LLM or prompt behavior | `references/llm-behavior-investigator.md` | Include for tool-selection, prompt-regression, context-pressure, or output-parsing hypotheses |
   | State consistency | `references/state-consistency-investigator.md` | Include for stale-read, replication, cache, event-ordering, or cross-service divergence hypotheses |
   | Infrastructure, config, or deployment | `references/infra-config-investigator.md` | Include for config, resource, deployment, provider, or environment hypotheses |
   | Controlled reproduction | `references/reproduction-experiment-investigator.md` | Include only when a hypothesis has a testable predicted outcome |

   Resolve every path relative to this skill's directory. Do not infer a path from a role label or invoke the retired standalone helper skill name.
...
3. **Dispatch investigators as parallel sub-agents**
   - Build each sub-agent's prompt from its selected reference path + incident context + scoped mandate + evidence schema
...
When all investigators complete (or budget exhausted), **dispatch the evidence-adjudicator sub-agent**, built from `references/evidence-adjudicator.md` (resolved relative to this skill's directory) + all branch evidence + the evidence schema:

1. Pass all branch evidence packets to the evidence-adjudicator sub-agent
...
> **Ownership:** The conductor orchestrates; the evidence-adjudicator sub-agent synthesizes. The conductor MUST NOT perform adjudication itself.
...
| Max total branches | 6 | Stop branching; dispatch the helper from `references/evidence-adjudicator.md` with current branches |
...
| Cost explosion | Budget > 80% with no verdict | Stop forking; dispatch the helper from `references/evidence-adjudicator.md` with partial findings |
...
| Adjudicator failure | Helper from `references/evidence-adjudicator.md` times out or returns malformed verdict | Retry once; if still failed, escalate with partial ranked evidence and mark adjudication as degraded |
...
**Helpers** (dispatched sub-agent prompts, not standalone skills): `references/timeline-trace-investigator.md`, `references/llm-behavior-investigator.md`, `references/state-consistency-investigator.md`, `references/infra-config-investigator.md`, `references/reproduction-experiment-investigator.md`, `references/evidence-adjudicator.md`.
