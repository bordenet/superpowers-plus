...
| GitLab's optional `^[Section]` form (leading `^`) is NOT excluded by the same filter -- reported as a bogus rule / false `DEAD RULE` | Disclosed, unfixed: pre-existing gap, not introduced by the anchoring/globstar fixes -- extend the exclusion regex to `^\s*(#\|$\|\^?\[)` to close it |
