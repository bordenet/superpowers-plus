---
name: cari-investigator-maintenance
source: superpowers-cari
description: Keeps the cari-investigator skill in sync with the Cari codebase by scanning git history for changes that affect investigation queries, log messages, constants, and DB schemas.
summary: "Use when: syncing cari-investigator skill with Cari codebase changes."
triggers: ["cari skill maintenance", "maintain cari skill", "update cari investigator skill", "sync cari investigator", "cari investigator drift check", "check cari skill for updates", "run cari maintenance"]
coordination:
  group: cari
  order: 1
  requires: []
...
# Cari Investigator Maintenance

> **Source:** superpowers-cari
> **Type:** Maintenance skill — read-only analysis of Cari monorepo git history

## When to Use

- Manually: "run cari maintenance" — before a sprint, after a release, or when investigations return unexpected results
- Auto-suggested: when cari-investigator loads and detects >10 new commits since `last_maintenance`

## Purpose

Scans Cari monorepo git history for changes affecting cari-investigator queries, log patterns, constants, or DB schemas. **NEVER auto-applies changes** — all updates require user approval.

## Prerequisites

- Cari monorepo cloned locally
- `CARI_REPO_PATH` set in `~/.codex/.env`
...
```bash
source ~/.codex/.env 2>/dev/null
source cari-investigator-maintenance/.maintenance-state 2>/dev/null \
...
if [[ -z "${last_maintenance:-}" ]]; then
  ...
  echo "Run from the superpowers-cari repo root (so cari-investigator-maintenance/.maintenance-state exists) or re-run install so the skill is present under ~/.codex/skills/."
  exit 1
fi
```
...
| `integration-platform/src/` | integration-platform.md, integration-platform-reference.md | SQS types, log messages, constants, metrics, Redis keys, polling logic, Authenticom lifecycle |
...
**Cross-reference step (mandatory):** For each detected change, grep the old value across all cari-investigator skill files to find exact line matches:

```bash
grep -rn "<old_value>" cari-investigator/ cari-investigator/references/
```
...
1. Run `sp-doctor` on cari-investigator to verify health
2. Update `.maintenance-state` file: `echo "last_maintenance=$(date +%Y-%m-%d)" > cari-investigator-maintenance/.maintenance-state`
