...
description: MSSQL schema reference for [[database]-prod] and [service]-prod. Tables, columns, join paths, and known gotchas.
---

# MSSQL Schema Map

> **Databases:** [[database]-prod] (MSSQL), [service]-prod (MSSQL)
...
### xcall_long[tracker] / xcall_short[tracker] — [Service] review results

| Column | Type | Notes |
|--------|------|-------|
| call[tracker]id | int (PK) | Auto-increment primary key |
| frn_callid | bigint | FK → xcall_long.callid (or xcall_short) — **⚠️ NOT `callid`** |
| frn[tracker]id | int | FK → [tracker].[tracker]id ([service]-prod) |
| frn[tracker]_optionid | int | Review answer option ID — **⚠️ NOT `frn_hresultid`. Lookup table not accessible via MCP connections.** |
...
### calltagdatum — Tag definitions (key [Product] IDs)

| Column | Type | Notes |
|--------|------|-------|
| calltagdatumid | int (PK) | Tag identifier |
| datum | varchar | **⚠️ Column is `datum`, NOT `description` or `name`.** Full text like "[Product] Service Scheduler Scheduled Appt" |
| isActive | bit | Whether the tag is currently active |

**Common [Product] tag IDs** (see `call-outcome-search/references/outcome-tags.md` for the full catalog):
...
| 2393 | ...Hang Up | [Product] detected hang-up |
...
### [tracker]_dnis — Per-DNIS [TRACKER] overrides

**⚠️ TWO DNIS columns exist.** Always use `frn_dnisid` for joins.

## [service]-prod

### [tracker] — Review category definitions

| Column | Type | Notes |
|--------|------|-------|
| [tracker]id | int (PK) | Category identifier |
| display_name | varchar | Human-readable name |
| [tracker]_question | varchar | Question reviewers answer |

### [tracker]_lskin — Per-account [TRACKER] configuration

| Column | Type | Notes |
|--------|------|-------|
| frn[tracker]id | int | FK → [tracker].[tracker]id |
...
xcall_long.callid → xcall_long[tracker].frn_callid, xcall_long[tracker].frn[tracker]id → [tracker].[tracker]id (⚠️ CROSS-DB: requires two separate queries — one to [[database]-prod], one to [service]-prod)
```
...
4. **Cross-DB JOINs impossible.** [[database]-prod] and [service]-prod are separate hosts.
...
The Postgres `reporting-prod.calls` table only contains calls handled by [Product]. Calls that ring directly to a dealer employee's desk phone (agent lines, outbound caller IDs) appear in MSSQL `xcall_long` but never touch [Product], so they have no Postgres transcript.

**How to tell if a DNIS is a [Product] line:**
- Check `calltag` for the call — if it has any `[Product]%` tags (e.g., "[Product] Receptionist Involved"), [Product] handled it
