...
description: AWS SSO setup, credential fetching, and database connection reference for [Product] investigations.
---
...
```
The [product]-investigator skill requires AWS access to query production databases and CloudWatch logs.

Prerequisites:
1. AWS CLI v2 installed (https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html)
2. Access to the [Company] AWS accounts (ask your manager if you don't have this)
...
Common profile names: `[product]-prod`, `[product]`, `telephony-prod`, `sre-prod`. Use whichever exists on your machine.

**If the user does not have AWS access** → inform them:

```
[Product] Investigator requires AWS access to query production databases and CloudWatch logs.
Without AWS access, this skill cannot run. Please contact your manager or DevOps
to request access to the [Company] AWS accounts.
```
...
> **⚠️ ALWAYS use the readonly secret.** The admin secrets (`[product]-rds-secret-production`, `[product]-rds-secret-dev-staging`) should NOT be used by this skill.
...
$secretJson = aws secretsmanager get-secret-value `
  --secret-id [product]-readonly-rds-production `
...
# Dev/Staging
# $secretJson = aws secretsmanager get-secret-value `
#   --secret-id [product]-readonly-rds-dev-staging `
...
Copy-paste this into your shell to create a one-liner for [Product] Postgres queries:

```bash
[product]_psql() {
  local profile="${1:-[product]-prod}" db="${2:-reporting-prod}" query="$3"
  local secret_json host port user pass
  secret_json=$(aws secretsmanager get-secret-value --profile "$profile" \
    --secret-id "[product]-readonly-rds-production" --query SecretString --output text)
...
# Usage:
# [product]_psql [product]-prod reporting-prod "SELECT count(*) FROM calls WHERE lskinid = 72965"
# [product]_psql [product]-prod config-prod "SELECT * FROM scheduler_configs LIMIT 1"
```
