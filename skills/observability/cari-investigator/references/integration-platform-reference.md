...
## Query IP6: Authenticom Lifecycle
...
1. `"Retrieving Authenticom Feed last updated since"` — polling cron discovers updated dealers
2. `"<lskin> - Sending Authenticom request"` — SQS message queued for this dealer
3. `"<lskin> - Sending Authenticom request status"` — feed request sent, status check queued (30s delay)
4. Status polling loop:
   - `Queued` / `InProgress` → re-queue with 30s delay, log `"Sending Authenticom request status"` again
   - `Ready` → paginate results (500/page), build VIN→advisor map, store in Redis (1yr TTL)
   - `Error` / `Purged` → `"<lskin> - Authenticom request status not valid (<status>) dropping request"` (WARN)
5. `"<lskin> - Error requesting Authenticom Feed"` — API call failed (ERROR)
...
| `AuthenticomFeedRequestStarted` / `AuthenticomFeedStatusCheck` / `AuthenticomFeedDataReceived` | Authenticom lifecycle |
| `AuthenticomVinMappingsStored` / `AuthenticomFeedFailure` | Authenticom results |
...
| `lastAuthenticomLoad` | Last Authenticom feed; determines date range for next request |
...
| Field | Purpose |
|-------|---------|
| `previousAdvisorEnabled` | Enables Authenticom VIN→advisor integration |
...
### "Authenticom advisor mapping isn't working"
1. **IP6** (Authenticom Lifecycle) — trace the full feed request → status → data flow
...
4. Check `previousAdvisorEnabled` flag in LoaderConfig — if `false`, Authenticom is skipped entirely for this dealer
5. Check `"Missing authenticom secrets"` — credentials not configured in Secrets Manager
...
| advisors | Daily (Authenticom) | <ts> | ✅/⚠️/N/A |
