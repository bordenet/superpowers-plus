...
triggers: ["investigate [product]", "debug [product]", "lookup account", "lookup lskinid", "[product] investigation", "what config does this dealer have", "check dealer config", "investigate call", "investigate account", "dealer performance", "booking rate", "call volume", "system health", "problem detection", "call drill-down", "what happened on this call", "RO value", "opcodes", "[[warm-transfer]]", "repeat callers", "containment rate", "acquisition sessions", "acquisitionportal", "agent logs", "what did the agent do", "model failover", "LLM error", "tool calls", "integration error", "why didn't it book", "slot unavailable", "data sync", "stale appointments", "polling broken", "CDK webhook", "DataFeedProvider", "Redis cache", "make model sync", "opcode sync", "integration platform", "polling health"]
...
- User says "investigate lskinid XXXXX" or "debug [product] for dealer Y"
...
- Data sync issues: "stale appointments", "polling broken", "CDK webhooks not arriving", "DataFeedProvider feed stuck"
...
source [company]/[product]-investigator-maintenance/.maintenance-state 2>/dev/null \
  || source ~/.codex/skills/[product]-investigator-maintenance/.maintenance-state 2>/dev/null \
  || last_maintenance="2026-03-19"
```

**If `[PRODUCT]_REPO_PATH` is set and the path exists:**
...
```
ℹ️ The [Product] codebase has <N> new commits since last skill maintenance (<last_maintenance>).
   Some investigation queries may be outdated.
   Say "run [product] maintenance" to check, or continue investigating.
```

**If `[PRODUCT]_REPO_PATH` is not set or path doesn't exist:**

Show a one-time warning, then proceed with the investigation:

```
⚠️ [Product] monorepo not found ([PRODUCT]_REPO_PATH not set in ~/.codex/.env).
   ...
   To enable drift checking, set [PRODUCT]_REPO_PATH in ~/.codex/.env
   or say "run [product] maintenance" to configure it.
```
...
This skill investigates [Product] platform issues by querying service databases directly. Each [Product] service has its own sub-skill file with schema references and SQL templates.
...
| integration-platform.md | integration-platform (CloudWatch) | DMS/scheduler data sync, Redis cache health, polling cycles, CDK webhooks, DataFeedProvider lifecycle |
...
```bash
mkdir -p /tmp/[product]/{creds,cw,db,node}
```
...
|-----------|-------------|---------|----------|
| `/tmp/[product]/creds/` | `db-{dbname}.json` | `db-config.json`, `db-reporting.json` | DB credentials from Secrets Manager |
| `/tmp/[product]/cw/` | `{service}-{query}.json` | `agent-q1-timeline.json`, `telephony-t3-lifecycle.json` | CloudWatch query results |
| `/tmp/[product]/db/` | `{service}-{query}.json` | `config-q1-account.json`, `reporting-q1-call.json` | DB query results |
| `/tmp/[product]/node/` | `node_modules/`, `package.json` | — | Node.js pg package install |

**Cleanup:** `rm -rf /tmp/[product]/`
...
- **Success** → fetch readonly creds: `aws secretsmanager get-secret-value --secret-id [product]-readonly-rds-production --profile telephony-prod`
...
|-------------|---------|--------|--------|
| Production | telephony-prod | [product]-readonly-rds-production | SELECT only |
| Dev/Staging | telephony-dev | [product]-readonly-rds-dev-staging | SELECT only |
...
```bash
cd /tmp/[product]/node && npm init -y && npm install pg  # package name is 'pg', require as: const { Client } = require('pg')
```

Or if the [Product] monorepo is cloned locally, use the existing package:
```bash
# Set [PRODUCT]_REPO_PATH to skip this — e.g. in ~/.codex/.env
# The postgres package is at: <[product]-repo>/config-service/node_modules/postgres
```

```javascript
// Option 1: standalone install (no codebase needed)
const { Client } = require('/tmp/[product]/node/node_modules/pg');
// Credentials from: /tmp/[product]/creds/db-config.json (config DB) or db-reporting.json (reporting DB)
const creds = JSON.parse(require('fs').readFileSync('/tmp/[product]/creds/db-config.json', 'utf8'));
...
"$AWS_CMD" logs get-query-results --query-id "$QID" --profile telephony-prod \
  --output json 2>/dev/null | tr -d '\r' > /tmp/[product]/cw/agent-q1-timeline.json

# Then parse from file
python3 -c "import json; d=json.load(open('/tmp/[product]/cw/agent-q1-timeline.json')); ..."
```
...
- Acquisition sessions, AcquisitionPortal, vehicle info -> reporting-service.md
...
- DataFeedProvider feeds, VIN-to-advisor mapping, feed status -> integration-platform.md
...
lskinid is the primary dealership identifier across ALL [Product] services. It links account info, all config types, call records, leads, and appointments. If the user provides a different identifier, resolve it to an lskinid first.
