...
SELECT
  ...
  aq.lead_id, aq.channel, aq.direction, aq.outcome,
...
WHERE c.agent_type = 'acquisition'
  AND c.lskinid = :lskinid
  -- Optional: AND aq.lead_id = :lead_id
  AND c.start_time >= NOW() - (:days * INTERVAL '1 day')
