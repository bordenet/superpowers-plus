...
description: Reporting DB queries for call/session data — single call drill-down, opcodes, RO value, [[warm-transfer]]s, repeat callers, acquisition sessions.
---
...
> **Credentials:** Same `[product]-readonly-rds-production` secret — see `references/aws-credentials.md`
...
| 6 | [[warm-transfer]] Stats by Dealer | lskinid, date range | "How are [[warm-transfer]]s performing for this dealer?" |
...
| 10 | Acquisition Session Lookup | lskinid or lead_id | "Show me AcquisitionPortal sessions for this dealer" |
...
> **For Queries 5-12 ([[warm-transfer]]s, repeat callers, language, acquisition, containment, phone lookup) and schema reference → see `references/reporting-advanced-queries.md`**
...
|---------|--------------|-------------------|
| Comprehension loop | Same assistant question appears 2+ times with different wording | [Product] misunderstood the caller; caller is repeating themselves |
| `[INTERRUPTED]` tag | Text contains `[INTERRUPTED]` in assistant content | Barge-in collision — [Product] and caller spoke simultaneously (latency or sensitivity issue) |
| `[Internal Error]` | Assistant content starts with `[Internal Error]` | Tool call or transfer failed; [Product] is recovering |
...
| Empty assistant content | `{"role":"assistant","content":null}` followed by tool call | Normal — [Product] is calling a tool before speaking |
...
| Ambiguous "yes" | Customer says "yes" to an either/or question | [Product] may misinterpret which option was selected |
...
| Value | Meaning | [Product] Issue? |
|-------|---------|-------------|
| `NULL` | Successful booking (no transfer needed) | No |
| `requested_transfer` | Caller explicitly asked for a human | Maybe — could be preference or [Product] failure |
...
| `pricing_question` | Caller asked about cost; [Product] can't answer | **Yes** — feature gap |
