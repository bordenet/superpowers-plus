...
> **Log Group:** `/aws/lambda/cari-config-post-all-production`
> **Profile:** cari-prod
...
aws logs start-query \
  --log-group-name "/aws/lambda/cari-config-post-all-production" \
...
```bash
aws logs get-query-results --query-id {query_id} --profile cari-prod
```
...
```bash
aws logs start-query \
  --log-group-name "/aws/lambda/cari-config-get-scheduler-production" \
  --start-time $START_TIME \
  --end-time $END_TIME \
  --query-string "fields @timestamp, @message
    | filter @message like /{lskinid}/
