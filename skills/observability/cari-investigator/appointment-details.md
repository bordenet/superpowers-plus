...
> **Systems:** [[database]-prod] (MSSQL) + CloudWatch (`[product]-agent-lambda-production`)
...
The `tz_datetime` column in MSSQL is when the **call happened**, not when the **appointment was booked for**. The booked appointment time lives only in the [Product] agent's tool call traces in CloudWatch.
...
3. **Call didn't reach the agent** — it may have been transferred before reaching [Product]
