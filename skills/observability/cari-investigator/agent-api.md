...
description: CloudWatch Insights queries for Cari agent Lambda logs — agent decisions, model failover, tool calls, LLM errors, integration debug.
---

# Agent API Sub-Skill

> **Log Group:** `/aws/lambda/cari-agent-lambda-production` (production) / `/aws/lambda/cari-agent-lambda-staging` (staging) / `/aws/lambda/cari-agent-lambda-dev` (dev)
