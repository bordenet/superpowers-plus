...
description: CloudWatch Insights queries for Cari integration-platform — DMS/scheduler data sync, Redis cache health, polling cycles, CDK webhooks, Authenticom lifecycle.
---

# Integration Platform Sub-Skill

Use this when the investigation is about **DMS data sync, appointment polling, Redis cache state, CDK webhooks, Authenticom feeds, make/model or opcode freshness**.
...
|--------|---------|---------|
| integration-platform | SQS queue | Routes messages by SQS `type` attribute: `BookingPolling`, `MakeModelReceived`, `OpcodesPolling`, `AuthenticomRequestFeedStart`, `AuthenticomFeedStatus` |
...
| `{lskin}:advisor:{vin}` | 1 year | VIN→advisor mapping (Authenticom) |
...
| Authenticom status poll delay | 30s (`DELAY_SECONDS`) | `authenticom.ts:4` |
| Authenticom retry count | 10 retries, 3s delay (`RETRY_COUNT`, `RETRY_DELAY`) | `authenticom.ts:2-3` |
| Authenticom page size | 500 (`MAX_PAGE_SIZE`) | `authenticom.ts:1` |
| Authenticom lookback | 1 day incremental (`DAYS_BACK`), 1 year initial (`YEARS_BACK`) | `authenticom.ts:6-7` |
| Authenticom VIN map TTL | 1 year (`EXPIRE_YEAR`) | `common.ts:20` |
...
| `AUTHENTICOM_REQUEST_FEED_START` | `AuthenticomRequestFeedStart` | `handleStartAuthenticomRequest` |
| `AUTHENTICOM_FEED_STATUS` | `AuthenticomFeedStatus` | `handleAuthenticomRequestStatus` |

### Authenticom Delivery Status Values (PascalCase)
...
- `"<lskin> - Sending Authenticom request status"` — Authenticom status queued
- `"<lskin> - Authenticom request status not valid (<status>) dropping request"` — Authenticom failed/expired
- `"<lskin> - Error requesting Authenticom Feed"` — Authenticom API error
...
|--------|---------------|----------------------|
| integration-platform | `/aws/lambda/cari-integration-platform-production` | `/aws/lambda/cari-integration-platform-dev-staging` |
| polling-service | `/aws/lambda/cari-polling-service-production` | `/aws/lambda/cari-polling-service-dev-staging` |
| cdk-webhook | `/aws/lambda/cari-cdk-webhook-production` | `/aws/lambda/cari-cdk-webhook-dev-staging` |
| CDK webhook API | `/aws/apigateway/cari-cdk-webhook-api-production` | `/aws/apigateway/cari-cdk-webhook-api-dev-staging` |
...
| IP6 | Authenticom Lifecycle | integration-platform | lskin | "What's happening with Authenticom for this dealer?" |
...
aws logs start-query \
  --log-group-name "/aws/lambda/cari-integration-platform-production" \
...
- `AuthenticomRequestFeedStart` — initiate Authenticom feed
- `AuthenticomFeedStatus` — poll Authenticom feed status

## Query IP4: Polling Health Check

**Log group:** `/aws/lambda/cari-polling-service-production` (or dev-staging)
...
**Log group:** `/aws/lambda/cari-cdk-webhook-production` (or dev-staging)
...
3. For Authenticom: show the full lifecycle state (which step is it on?)
...
> **For Queries IP6-IP9 (Authenticom lifecycle, bulk error scan, CDK load trace, CloudWatch metrics), LoaderConfig schema, booking eligibility, investigation scenarios, and report template → see `references/integration-platform-reference.md`**
