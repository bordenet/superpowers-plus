...
If the user says multiple people were testing, or if the ANI search returns 0 results, fall back to searching by [Product] test account name (see Step 3 alternative query).
...
**Alternative: Search by [Product] test account (when multiple testers use different phones):**
...
JOIN dnis d ON x.cf_frn_dnisid = d.dnisid
JOIN lskin l ON d.add_lskinid = l.lskinid
WHERE l.refname LIKE '%[Product] Phone Assist Test%'
  AND x.tz_datetime >= '{start_datetime}'
  AND x.tz_datetime < '{end_datetime}'
ORDER BY x.tz_datetime DESC
```
...
- *"Want me to look up [TRACKER] tags for any of these calls?"* (→ use `[tracker]-lookup`)
...
- `[tracker]-lookup` — check [TRACKER] tagging for a call
