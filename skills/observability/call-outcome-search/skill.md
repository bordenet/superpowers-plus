...
description: "Find Cari calls by booking outcome, time window, service lane, and dealer. Resolves dealer names to lskinids, maps outcome descriptions to calltag IDs, converts local time to UTC, and optionally identifies service lane from scheduler config."
summary: "Use when: investigating specific booking outcomes at a dealer within a time window. Skip when: general call search (use call-search), full investigation (use cari-investigator)."
...
> **Source:** superpowers-cari
> **Domain:** Cari booking investigation
> **Databases:** callmeasurement-prod (MSSQL), config-prod (Postgres RDS)

Find calls that produced a specific Cari outcome (e.g., "Scheduled Appt") at a dealer within a time window. Optionally filter by service lane (Express, Main Shop).
...
See `references/outcome-tags.md` for the full Cari Service Scheduler tag catalog (18 calltagdatumid values from `callmeasurement-prod.dbo.calltagdatum`).
...
Cross-reference the lane ID against the Cari agent trace logs (CloudWatch) for the specific call IDs to confirm which lane the appointment was booked into:
...
- Recommend next steps (e.g., "review agent traces for these call IDs" via `cari-investigator`)
