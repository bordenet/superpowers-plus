...
If the most recent row is older than yesterday, the ETL pipeline is lagging. **Use `xcall_short` instead of `xcall_long`** for all queries below. Replace `xcall_long` → `xcall_short` and `xcall_long[tracker]` → `xcall_short[tracker]` in your queries.
