...
If `CI=true` is in the environment, OR if running in a non-interactive context detected by the absence of a TTY on stdout (`[ ! -t 1 ]` in shell; `!process.stdout.isTTY` in Node), proceed automatically without waiting. Before starting Phase 1, emit the reason that triggered auto-proceed: CI set → `"Auto-proceeding (non-interactive: CI=<value>) — verify scope before treating report as authoritative."`; TTY absent → `"Auto-proceeding (non-interactive: TTY=absent) — verify scope before treating report as authoritative."`; both → `"Auto-proceeding (non-interactive: CI=<value>, TTY=absent) — verify scope before treating report as authoritative."` Note the auto-proceed in the report header. SSH users running without PTY allocation (`ssh -T`) should pass `--no-confirm` to make non-interactive intent explicit.
