---
name: update-superpowers-cari
source: superpowers-cari
triggers: ["update my cari superpowers", "update superpowers", "upgrade superpowers", "pull superpowers", "refresh superpowers", "update skills", "upgrade skills", "sp-update", "superpowers-update"]
anti_triggers: ["install superpowers", "uninstall superpowers"]
description: "Pull latest changes across all superpowers repos (including cari + recruiting), run the full install chain, and fix any issues found by sp-doctor. Overrides the base update-superpowers from superpowers-callbox."
...
# Update My Cari Superpowers

> **Override:** This skill replaces the base `update-superpowers` from `superpowers-callbox`. It adds superpowers-cari and superpowers-recruiting repos, and runs the install from superpowers-cari (which cascades through the full chain).
...
# 1d. superpowers-cari (discovered from .env)
cd "$CARI_SOURCE_DIR" && git pull origin main --ff-only

# 1e. superpowers-recruiting (discovered from .env)
cd "$SPR_SOURCE_DIR" && git pull origin main --ff-only
```

**Rules:**
- Always `source ~/.codex/.env` first — paths come from `SPC_SOURCE_DIR`, `CARI_SOURCE_DIR`, `SPR_SOURCE_DIR`, etc.
...
```bash
cd "$CARI_SOURCE_DIR" && bash install.sh --upgrade
```

This cascades: `obra/superpowers → superpowers-plus → superpowers-callbox → superpowers-cari` (superpowers-recruiting is pulled in Step 1 but has no install chain dependency — its skills are standalone)
