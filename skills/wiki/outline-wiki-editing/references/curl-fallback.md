...
`https://wiki.int.callbox.net/api`
...
curl -s -X POST "https://wiki.int.callbox.net/api/documents.info" \
...
curl -s -X POST "https://wiki.int.callbox.net/api/documents.update" \
  -H "Authorization: Bearer $OUTLINE_API_KEY" \
  -H "Content-Type: application/json" \
...
EXISTING=$(curl -s -X POST "https://wiki.int.callbox.net/api/documents.list" \
  -H "Authorization: Bearer $OUTLINE_API_KEY" \
  -H "Content-Type: application/json" \
