...
main() {
    ...
    # When install.sh --with-proxy has flipped OUTLINE_API_URL to the local
    # proxy, prefer OUTLINE_REAL_* so snapshots speak raw Outline REST
    # (Authorization: Bearer), bypassing the proxy's X-Proxy-Token gate.
    if [[ -n "${OUTLINE_REAL_API_URL:-}" ]]; then
        OUTLINE_API_URL="$OUTLINE_REAL_API_URL"
    fi
    if [[ -n "${OUTLINE_REAL_API_KEY:-}" ]]; then
        OUTLINE_API_KEY="$OUTLINE_REAL_API_KEY"
    fi
    ...
    http_code=$(curl -s --connect-timeout 10 --max-time 30 -w '%{http_code}' -o "$_tmpresponse" \
        -X POST "${OUTLINE_API_URL}/documents.info" \
        -H "Authorization: Bearer ${OUTLINE_API_KEY}" \
        -H "Content-Type: application/json" \
        -d "$payload") || {
        echo "Error: curl failed (network error or timeout after 30s)" >&2
        exit 3
    }
